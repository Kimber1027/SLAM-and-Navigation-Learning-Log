课程链接 [第05讲B IMU误差模型_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1nR4y1E7Yj?spm_id_from=333.788.videopod.episodes&vd_source=0ecf80afdba926bd66e4203eb7017f51&p=10)、[第06讲A IMU误差测试与标定_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1nR4y1E7Yj?spm_id_from=333.788.videopod.episodes&vd_source=0ecf80afdba926bd66e4203eb7017f51&p=11)
...
![](./image/Allan标准差曲线.png)
纵轴为Allan标准差，横轴为时间。
为了方便观察和分析，采用双对数曲线（横轴和纵轴均采用对数刻度）。

因为噪声大都满足：$σ(τ)∝τ^k$，所以在双对数坐标下就是一条斜率为k的直线。
**不同噪声会表现为不同斜率k的线段。**

| 双对数斜率 | 噪声类型   | 理解                                                              |
| ----- | ------ | --------------------------------------------------------------- |
| -1    | 量化噪声   | 传感器输出被数字化时，存在最小分辨率，短时间平均时这种离散误差很明显。随着平均时间增大，这种误差下降得很快，表现为最陡的部分。 |
| −1/2  | 白噪声    | 白噪声是随机快速抖动。平均时间越长，随机噪声互相抵消得越多，但下降没量化噪声那么快。                      |
| 0     | 零偏不稳定  | 误差不明显随平均时间变化，主要体现为传感器偏置在慢慢漂、但没有明显按某个幂次增长或下降。                    |
| 1/2   | 速率随机游走 | 这类噪声在短时间看不太明显，但随着时间拉长，低频漂移不断累积，误差越来越大。                          |
| 1     | 速率斜坡   | 比如温漂、老化、系统性缓慢漂移，时间越长影响越大，而且比随机游走长得更快。                           |
- **Correlated Noise**：相关噪声，表现为局部鼓包、隆起，说明存在时间相关性。
- **Sinusoidal**：正弦干扰/周期扰动，表现为局部波纹、起伏，说明存在周期干扰。
